# Componenti e Blocchi di Contenuto in SaluteOra

## Introduzione

Questo documento descrive i componenti e i blocchi di contenuto disponibili in SaluteOra per la gestione dei contenuti statici tramite file JSON. È fondamentale utilizzare solo i componenti esistenti e rispettare la struttura corretta per evitare errori di rendering.

## Struttura dei File JSON

I contenuti statici in SaluteOra sono gestiti tramite file JSON nella directory:
```
...
Rispettare la struttura dei blocchi di contenuto e utilizzare solo componenti esistenti è fondamentale per garantire il corretto funzionamento del sistema di gestione dei contenuti di SaluteOra. In caso di dubbio, è sempre meglio esaminare i file JSON esistenti per comprendere la struttura e i componenti disponibili.
