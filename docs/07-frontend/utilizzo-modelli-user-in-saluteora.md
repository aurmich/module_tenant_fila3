# Utilizzo Corretto dei Modelli User in SaluteOra

## Introduzione

SaluteOra utilizza un'architettura modulare con Laravel Modules, dove ogni modulo ha la propria struttura e namespace. Questo documento descrive le best practices per l'utilizzo corretto dei modelli User all'interno dell'applicazione.
...
Questo approccio è errato perché SaluteOra utilizza una struttura modulare dove il modello User è definito all'interno del modulo User.
...
## Autenticazione in SaluteOra

### Utilizzo di Passport invece di Sanctum

SaluteOra utilizza Laravel Passport per l'autenticazione API invece di Sanctum. Questo è importante da ricordare quando si implementano funzionalità di autenticazione:
...
Passport è preferito in SaluteOra perché offre:
...
4. **Utilizzare Passport per l'Autenticazione**: Ricordare che SaluteOra utilizza Passport per l'autenticazione API.

5. **Verificare i Namespace**: Controllare sempre che i namespace utilizzati siano corretti e rispettino la struttura modulare di SaluteOra.

## Conclusione

L'utilizzo corretto dei modelli User in SaluteOra è fondamentale per mantenere un'architettura pulita e modulare. Seguendo le best practices descritte in questo documento, è possibile evitare errori comuni e garantire che il codice sia manutenibile e scalabile.
