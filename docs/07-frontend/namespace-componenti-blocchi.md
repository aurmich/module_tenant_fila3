...
Questo documento spiega come gestire correttamente i namespace nei componenti a blocchi in SaluteOra, con un focus particolare sulla risoluzione di problemi comuni.

## Namespace pub_theme vs one

In SaluteOra, il tema pubblico viene referenziato con il namespace `pub_theme::` nei file di configurazione e nei file JSON, indipendentemente dal nome effettivo del tema (che può essere "One" o altro).
