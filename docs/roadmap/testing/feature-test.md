...
L'implementazione di feature test per le funzionalità critiche della piattaforma SaluteOra è attualmente in fase di pianificazione (0%). Questa componente è fondamentale per garantire la corretta funzionalità delle parti più importanti dell'applicazione attraverso test automatici che verificano il comportamento di interi moduli e flussi di lavoro.
