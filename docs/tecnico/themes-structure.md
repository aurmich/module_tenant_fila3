# Struttura dei Temi in SaluteOra

## Posizionamento Corretto dei Temi

Nel progetto SaluteOra, è fondamentale mantenere una chiara separazione tra moduli funzionali e temi. I temi devono essere posizionati nella directory `Themes`, non nella directory `Modules`.
...
Un'importante considerazione sulla struttura dei temi in SaluteOra è che tutta la logica di gestione dei temi è già implementata dal **modulo Cms**. Non è necessario creare un service provider dedicato (come `ThemeServiceProvider`) poiché il modulo Cms si occupa già di:
